...
export class Patrol {
    ...
    patrolCompute() {
        if (patrolInstances._patrol.executePatrol && !game.paused && !game.combat?.started && patrolInstances._patrol.started) {
            ...
            canvas.scene.updateEmbeddedDocuments("Token", updates, { movement: { [update._id]: { autoRotate } } });
    ...
    getValidPositions(token, occupiedPositions) {
        let validPositions = [];
        this.getDirections(token).forEach((d) => {
            if (
    ...
    getDirections(token) {
        return [
            this.randomPointInPolygon(token),
            this.randomPointInPolygon(token),
            this.randomPointInPolygon(token),
            this.randomPointInPolygon(token)
        ]
    ...
    randomPointInPolygon(token) {
        if (!token.patrolPolygon) return;

        const pts = token.patrolPolygon.points;
        ...
        for (let i = 0; i < indices.length; i += 3) {
            const ax = pts[indices[i]   * 2], ay = pts[indices[i]   * 2 + 1];
            const bx = pts[indices[i+1] * 2], by = pts[indices[i+1] * 2 + 1];
            const cx = pts[indices[i+2] * 2], cy = pts[indices[i+2] * 2 + 1];
        ...
        const triangle = triangles.find(t => (r -= t.area) <= 0) ?? triangles[triangles.length - 1];
        ...
        const topLeft = {
            x: triangle.ax + u * (triangle.bx - triangle.ax) + v * (triangle.cx - triangle.ax),
            y: triangle.ay + u * (triangle.by - triangle.ay) + v * (triangle.cy - triangle.ay),
        };
        ...
        return {
            ...
            y: topLeft.y,
            center: {
                x: topLeft.x - token.tokenDocument.w / 2,
                y: topLeft.y - token.tokenDocument.h / 2,
            }
        }
    }

    getDirections_old(token) {
